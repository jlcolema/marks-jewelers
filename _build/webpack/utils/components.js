...
const stylesDir = path.join(path.join(rootDir, 'src'), 'css');
// const scriptsDir = path.join(path.join(rootDir, 'src'), 'js');
...
module.exports = function () {
  ...
  fs.readdirSync(componentsStyles).forEach(file => {
    ...
    if (!componentName.startsWith('_') && !componentName.startsWith('.') && extension === '.scss') {
